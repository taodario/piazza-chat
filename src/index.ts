...
interface PiazzaPost {
  ...
  function score(post: PiazzaPost, q: string): number {
	const postText = `${post.subject || ""} ${stripHtml(post.content || "")}`;
	const postTokens = new Set(tokenize(postText));
	const queryTokens = tokenize(q);
	let matches = 0;
	for (const t of queryTokens) if (postTokens.has(t)) matches++;
  ...
  ${body}${extras ? `\n${extras}` : ""}`;
		  ...
		  // Return both answer and post details
		  return json({
			query: q,
  ...
  // --- HTML front-end (visible in browser) ---
