...
COURSE_ID = "meu2e1usaod412"  # your Piazza course ID

def save_partial(posts):
    with open("piazza_data_partial.json", "w") as f:
        json.dump(posts, f, indent=2)
    print(f"💾 Saved partial progress ({len(posts)} posts)")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "too fast" in str(e):
                print(f"⚠️ Rate limit hit on {post_id}. Backing off {backoff}s...")
...
def main():
    ...
    # Get post list summary only (no full data yet)
    all_posts = course.get_feed(limit=5000)["feed"]
    print(f"🧾 Found {len(all_posts)} total post summaries")
    ...
    for i, summary in enumerate(all_posts):
        ...
        print(f"✅ [{i+1}/{len(all_posts)}] {simplified['subject'][:70]}")

        if len(posts) % 10 == 0:
            save_partial(posts)

        # always sleep politely between posts
        time.sleep(random.uniform(3, 6))

    with open("piazza_data.json", "w") as f:
        json.dump(posts, f, indent=2)

    print(f"🎉 Done! Exported {len(posts)} full threads to piazza_data.json")
